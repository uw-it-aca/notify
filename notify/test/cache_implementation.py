...
class UICacheTest(TestCase):
    def test_cached_404(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS=(
                'notify.cache_implementation.UICache')):
            ...
            query = CacheEntryTimed.objects.filter(
                service="sws", url=error_url)

            # Make sure we don't cache errors for more than 5 minutes
            cache_entry = query[0]
            cache_entry.time_saved = (
                cache_entry.time_saved - timedelta(minutes=5))
            cache_entry.save()
    ...
    def test_sws_resources(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS=(
            ...
            query = CacheEntryTimed.objects.filter(
                service="sws", url=should_be_cached_url)

            # Make sure that the cached entry is still used 8 hours later.
            cache_entry = query[0]
            cache_entry.time_saved = (
                cache_entry.time_saved - timedelta(hours=8))
            cache_entry.save()
            ...
            query = CacheEntryTimed.objects.filter(
                service="sws", url=no_cache_url)

            # Make sure that the cached entry is not used 1 minute later
            cache_entry = query[0]
            cache_entry.time_saved = (
                cache_entry.time_saved - timedelta(minutes=1))
            cache_entry.save()
    ...
    def test_nws_resources(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS=(
            ...
            should_be_cached_url = ('/notification/v1/channel?tag_year=2013'
                                    '&tag_quarter=autumn&max_results=1')
            no_cache_url = ('/notification/v1/template/9a2cfb4c-8120-'
                            '45c6-a7a6-9eac7cf95050')
            cache = UICache()
            ...
            query = CacheEntryTimed.objects.filter(
                service="nws", url=should_be_cached_url)
